  ...
  {                     \
    ...
    void ProcessPacket(struct Packet *p)
    {
      if (p->address == 0xFF || p->address == deviceId || true) // Yay, it's for me
      {
        ...
        {
          uint8_t base = 1 + 59 * 3;
          fill_solid(rightColors, 1, CRGB((uint8_t)(p->message[base]), (uint8_t)(p->message[base + 1]), (uint8_t)(p->message[base + 2])));

          uint8_t leftBase = 1;
          fill_solid(leftColors, 1, CRGB((uint8_t)(p->message[leftBase]), (uint8_t)(p->message[leftBase + 1]), (uint8_t)(p->message[leftBase + 2])));
          /*for (int i = 0; i < 26; i++)
          {
